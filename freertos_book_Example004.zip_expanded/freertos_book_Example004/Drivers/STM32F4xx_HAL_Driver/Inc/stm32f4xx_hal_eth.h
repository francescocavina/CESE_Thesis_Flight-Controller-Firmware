...
typedef struct {
    ...
    FunctionalState ForwardUndersizedGoodFrames; /*!< Enables or disables the Rx FIFO to forward Undersized frames (frames with no Error
    ...
    FunctionalState SecondFrameOperate; /*!< Selects or not the Operate on second frame mode, which allows the DMA to process a second
 ...
 */
typedef void (*pETH_rxLinkCallbackTypeDef)(void ** pStart, void ** pEnd, uint8_t * buff, uint16_t Length); /*!< pointer to an ETH Rx Set App Data Function */
/**
 *
 */
 ...
 * @brief  HAL ETH Tx Free Function definition
 */
typedef void (*pETH_txPtpCallbackTypeDef)(uint32_t * buffer, ETH_TimeStampTypeDef * timestamp); /*!< pointer to an ETH Tx Free function */
/**
 *
 */
 ...
 */
#define __HAL_ETH_DMA_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__) (((__HANDLE__)->Instance->DMAIER & (__INTERRUPT__)) == (__INTERRUPT__))
 ...
 */
#define __HAL_ETH_DMA_GET_IT(__HANDLE__, __INTERRUPT__) (((__HANDLE__)->Instance->DMASR & (__INTERRUPT__)) == (__INTERRUPT__))
 ...
 */
#define __HAL_ETH_MAC_GET_IT(__HANDLE__, __INTERRUPT__) (((__HANDLE__)->Instance->MACSR & (__INTERRUPT__)) == (__INTERRUPT__))
...
#define __HAL_ETH_GET_PTP_CONTROL(__HANDLE__, __FLAG__)    (((((__HANDLE__)->Instance->PTPTSCR) & (__FLAG__)) == (__FLAG__)) ? SET : RESET)
