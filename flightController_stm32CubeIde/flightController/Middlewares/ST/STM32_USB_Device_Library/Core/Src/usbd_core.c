...
uint8_t USBD_CoreGetEPAdd(USBD_HandleTypeDef * pdev, uint8_t ep_dir, uint8_t ep_type) {
    ...
    for (idx = 0; idx < pdev->tclasslist[pdev->classId].NumEps; idx++) {
        if (((pdev->tclasslist[pdev->classId].Eps[idx].add & USBD_EP_IN) == ep_dir) && (pdev->tclasslist[pdev->classId].Eps[idx].type == ep_type) && (pdev->tclasslist[pdev->classId].Eps[idx].is_used != 0U)) {
